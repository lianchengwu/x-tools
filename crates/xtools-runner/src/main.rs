...
use xtools_protocol::*;
use xtools_runtime::PluginInstance;
use xtools_ui::boot::{
...
fn find_plugin_wasm(arg: &str) -> Option<PathBuf> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plugin_arg = args.get(1).cloned().unwrap_or_else(|| "time".to_string());
    ...
    // Helper macro / closure for event handling
    let handle_event = {
